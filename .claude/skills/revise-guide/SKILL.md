...
- Suggested changelog: **one short sentence of at most 75 characters — this is a hard constraint, not a stylistic preference.** 75 is the measured width that still fits on a single line of the rendered table, so count the characters rather than eyeballing the length, and never let an entry run onto a second line. It still has to be concrete about what actually changed since the last row — based on the edits made this session, or `git diff` against the guide file if the session doesn't have that context — so "updates" and other generic placeholders don't qualify.
  ...
  This is the one field where the user's override still has a shape to respect. If a changelog over 75 characters is dictated, tighten it to fit rather than pasting it in — and say that you did.
...
- The `## Revision History` table isn't a cramped, equal-width block — Version/Date stay narrow and Changelog fills the remaining width with left-aligned text. Every changelog entry sits on a single line; a wrapped entry means step 2's 75-character constraint was missed and the fix is to shorten the sentence, not to widen the column.
