...
This skill, and every `*-work-item` skill, stays project-agnostic: no project-specific file names, feature names, tech stack, or other-skill names — only the generic vocabulary already used throughout this skill (work item, feature, task, plan file, permanent doc, slug). If a concrete example is ever needed, invent a clearly hypothetical one rather than naming something that actually exists in this project. This isn't just workflow hygiene: every skill in a repo except that project's own listed skills (see its `CLAUDE.md`) is generic tooling that can be renamed, replaced, or deleted independently of the project it's checked into — this skill included — so a hard-coded reference to one anywhere would go stale silently.
...
- `docs/technical/<name>.md` (`<name>` = kebab-case slug of the topic) — one file per granular technical decision. Self-contained and leaf-only: never links to another file under `docs/technical/`, including `technical.md` itself — the index is the only inbound link, so no leaf file ever needs updating when the index changes. A general coding convention that would hold in any project using this stack (how to write the code, as opposed to what was decided) doesn't belong here — see "Recording a technical decision" below for the line between that and a project-specific design-system convention, which does.
...
Before filing something under `docs/technical/`, check what kind of fact it actually is: a standing decision about *this* project (architecture, stack, layout, guiding principles) belongs here; a rule about *how to write code* that would hold in any project using this stack does not — even if no dedicated file currently owns it. Don't invent a `docs/technical/<name>.md` topic for a generic coding convention just because it needs a home; flag it instead (a `docs/process-notes.md` entry, per its own description above) so it gets enforced wherever this project's conventions actually live. The line isn't "code vs. not code" — a project-specific design-system convention (exact styling, spacing, or layout rules particular to this app's own UI, not general framework practice) is a standing decision about this project and does belong under `docs/technical/`, same as architecture or layout.
...
Phrase a row's summary as *when to read the file*, not a table of contents of what's inside — "Read when doing X" rather than "Covers A, B, and C." The index exists so a reader (human or AI) can decide relevance without opening every topic file; a trigger condition serves that directly, while a content outline still forces the reader to infer whether it applies. This holds for every row, not only new ones — reword an existing row the same way whenever it's touched for an unrelated reason.

A `docs/technical/<name>.md` entry states the *why* behind a standing decision and *when to apply it* — never *when it was made* or *who currently relies on it*. Leave out build order, which lab or module first introduced the pattern, which labs currently use it, and "an earlier version of this project did X, then switched to Y" narratives. These are point-in-time facts that go stale the moment a lab is added, renamed, or refactored, which is exactly the staleness this file structure exists to avoid — state the rule and its rationale as a standing fact, not as a change log.
