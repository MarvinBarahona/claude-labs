...
Never edit a permanent doc outside this work item's own plan file during this phase — not a dependency's `docs/features/<slug>.md` or `docs/shared/<slug>.md`, not `docs/technical/*.md`, not `README.md`/`CLAUDE.md`/a skill file — even if the plan's own to-do list calls for updating one in place (e.g. a follow-on whose `**Target doc:**` already exists). Implementation can still shift after a doc like that is written but before it's actually compiled/tested, forcing the same doc to be rewritten twice. Leave it untouched and record the needed update as a development note instead (see below); it gets applied once during `graduate-work-item`, after the implementation it describes has settled.
...
- **Automated scenarios** — implement and run these directly, using whichever in-container command the plan file's own citation calls for (see `plan-work-item`'s "Automated vs. manual test scenarios") — never go looking for that command in the project's own docs independently; this skill reads only the plan file, per "Read first" above. This is the only test execution this skill performs itself.
...
Append a `## Development notes` section to the work item's own plan file (create it if it doesn't exist yet) recording anything that deviated from the plan, any ad hoc decision made during implementation, or anything a future work item should know about. Tag each note by kind so `graduate-work-item` can route it correctly: a technical decision made during implementation (tag it for `docs/technical/`); a permanent doc — a dependency's, or this work item's own `**Target doc:**` — that needs updating now that implementation has settled (tag it as a permanent-doc update, per "Phase 1 — Execute" above); a fact that would change what `README.md`, `CLAUDE.md`, or another skill should claim (tag it as a non-owned-file suggestion); or a coding-convention/process observation. This skill never edits `docs/technical/`, another work item's permanent doc, `README.md`, `CLAUDE.md`, or skill files itself — only records the observation for later review.
