...
**For a feature:**
1. Fresh: create `docs/features/<slug>.md` from `docs/planning/feature-<slug>.md`. Follow-on: open the existing `docs/features/<target-slug>.md` and merge this work item's changes into it. Either way, keep only lasting reference material — what the feature does, how to use it, key API/behavior decisions. Drop planning-only content: build-order position, dependency notes, open questions, test scenarios, development notes. Write it for a reader who will later treat this feature purely as a dependency to build on — include the interface/behavior detail that reader needs, not just a human-facing summary. For a follow-on, that means the merged doc describes the feature as it now behaves, not a changelog of what changed and when — history lives in `docs/status.md`'s rows, not in the permanent doc's prose. One exception to "drop planning-only content": if the plan file carries a `**Nav position:**` line, carry it forward into the permanent doc unchanged — the app still needs it after the feature ships to render its nav position correctly. Most projects have no navigation concept at all, per `writing-docs`' "Feature nav position" — if there's no such line, there's nothing to carry forward.
