...
- **Automated** — anything a test can assert programmatically. If this project documents its own testing strategy (categories of test, how each is run, how mocking/credentials work), it would surface via "Read first" above like any other project-wide topic — follow it, and cite the exact category/section this work item's automated scenarios draw on, per "A work item's plan file must be self-contained" above. `build-work-item` implements and runs these itself, in-container, using whichever command that citation names, rather than re-deriving it from project docs independently.
- **Manual** — anything that needs a person looking at the actually-running app to judge (layout, styling, an interaction only visible in a real browser, a real multi-step click-through) and can't be asserted by a test. Write each as a concrete, numbered step ("do X, expect Y") precise enough for the user to execute directly against their own locally-running instance of the app (however this project is normally started — see its own setup docs) — `build-work-item` hands this list to the user to run rather than running it itself by default; see that skill's "Manual test scenarios" section.
