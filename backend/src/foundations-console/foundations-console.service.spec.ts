...
describe('FoundationsConsoleService', () => {
  let fakeClient: FakeAnthropicClient;
  let service: FoundationsConsoleService;
  ...
  beforeEach(async () => {
    ...
    service = moduleRef.get(FoundationsConsoleService);
  });

  describe('createTurn (non-streaming /messages)', () => {
    ...
    it('shapes the fake response into the envelope, with no `calls` array', async () => {
  ...
  describe('streamTurn (streaming /messages)', () => {
    it('forwards the fake client stream events verbatim, followed by exactly one terminal turn_complete frame', async () => {
      ...
      const frames: FoundationsStreamFrame[] = [];
  ...
  describe('runStructuredDemo (/structured)', () => {
    it('calls createMessage() with the fixed schema and shapes the parsed result into the envelope', async () => {
      const structuredPayload = {
        summary: 'Great feedback overall',
      ...
      const dto: StructuredDemoDto = {
        modelChoice: 'default',
        input: 'Some review text',
      };
      const envelope = await service.runStructuredDemo(dto);

      const sentCall = fakeClient.recordedCalls[0];
      expect(sentCall.output_config).toEqual({
        format: {
          type: 'json_schema',
          schema: {
            type: 'object',
