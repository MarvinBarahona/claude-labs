...
- `streamEvents` renders incrementally: a caller replaces it wholesale with a new array reference as events arrive (e.g. appending to a signal-held array) — `OnPush` change detection picks up the new input each time, with no internal event buffer of the component's own. Because the whole `InspectorCall` value is replaced on each update rather than merged, the call site that applies a streaming turn's *final* envelope must explicitly re-include the already-accumulated `streamEvents` in that same update — omitting the field there (even though earlier per-event updates set it) silently drops the stream-event log the instant the turn completes, rather than leaving it in place.
