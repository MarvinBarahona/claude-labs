...
Each project has a working test command using only placeholder environment values — see `testing-strategy.md` for the strategy and `CLAUDE.md` for the actual commands (`docker compose run --rm backend npm test`, `docker compose run --rm backend npm run test:e2e`, `docker compose run --rm frontend npm test -- --watch=false`).
