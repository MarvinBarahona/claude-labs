...
- [`prod-docker.md`](../shared/prod-docker.md), "Interface" — the second Docker/Compose runtime (a single container where the backend serves the compiled frontend, started via `docker compose -f docker-compose.prod.yml up`, distinct from the dev `docker-compose.dev.yml`, which itself now also requires an explicit `-f docker-compose.dev.yml`). Cited for context only, not for duplicated verification — see "Open questions."
...
- **Real mode** — a real `ANTHROPIC_API_KEY` (and other credentials) in `backend/.env`; every external call goes out for real. This is how the project owner actually uses the app day to day for manual testing, and how anyone else it's shared with is expected to run it: add keys to `.env`, `docker compose -f docker-compose.dev.yml up`, nothing else.
...
Sits after [`test-doubles.md`](../shared/test-doubles.md) and [`app-shell.md`](../shared/app-shell.md), and before the first feature, Foundations Console (see `status.md` for current position) — both dependencies above (the fakes to bind, and the header to carry the banner) need to already exist. The prod-docker task sits earlier in `status.md` too, but that ordering is for planning-context accuracy (see "Depends on"), not because this task's own build or test work needs it done first.

## Test scenarios

- [ ] With `FAKE_MODE=true` and only placeholder credentials in `.env`, `docker compose -f docker-compose.dev.yml up` boots the backend successfully (startup validation still only checks presence, per `env-config.md`).
...
These scenarios are exercised once, under dev's `docker compose -f docker-compose.dev.yml up` — see "Open questions" for why they aren't duplicated under the prod-docker task's runtime too.
...
- [ ] Confirm `docker compose -f docker-compose.dev.yml up` with `FAKE_MODE=true` and placeholder-only credentials boots and serves a working demo end to end with no outbound network call.
