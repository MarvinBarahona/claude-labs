...
- [`task-api-error-handling.md`](task-api-error-handling.md) (`Draft`) — its normalized "external call failed" exception class is what `RealAnthropicClient` throws when a real Anthropic call fails (bad key, rate limit, etc.), instead of letting the raw SDK error propagate unshaped. Build (or at least fully plan) that task before or alongside this one's `RealAnthropicClient` implementation.
