...
- [x] Implement the lab-local `ArxivClient` (`RealArxivClient`, `FakeArxivClient`, `fakeSwitchProvider()` wiring), per "Depends on" above.
- [x] Implement `POST /session` (fetch + in-memory session creation).
- [x] Implement the document content-block attachment (first-ask-only) with citations enabled and the cache boundary.
- [x] Implement `file_id` reuse across asks in `files-api` mode.
- [x] Implement the text-editor tool loop against the session's single `/notes.md` virtual file, including the `str_replace` uniqueness/`is_error` handling and line-numbered `view` output.
- [x] Implement citation flattening into the response shape.
- [x] Implement streaming (`tool_call_start`/`tool_call_result`/`turn_complete`), reusing Live Tool-Use Console's SSE plumbing.
- [x] Implement `404` handling for an unknown `sessionId`.
- [x] Build the frontend: arXiv-ID form, chat transcript with citation markers, delivery-mode toggle, notes side panel.
- [x] Write this lab's in-app doc (`write-lab-doc`).
- [x] Add the browser E2E spec (`e2e/tests/document-research-assistant.spec.ts`) — per [`frontend-browser-e2e-tests.md`](../shared/frontend-browser-e2e-tests.md)'s "Specs", only once the in-app doc above already exists, since the spec's docs-panel assertion needs real rendered content to check.
- [x] Wire `DocumentResearchAssistantModule` (imports `ModelConfigModule`, `AnthropicClientModule`, `EnvelopeBuilderModule`, `ContentBlockBuilderModule`, `CachingLayerModule`).
...
- **[permanent-doc: `anthropic-client.md`]** `AnthropicClient.createMessage()`/`streamMessage()` gained an optional trailing `betas?: string[]` parameter. Reason: the Files-API `file_id` document/image source shape only exists on the SDK's **beta** Messages types (`Anthropic.Beta.Messages.*`) — the stable `DocumentBlockParam.source` union (`Base64PDFSource | PlainTextSource | ContentBlockSource | URLPDFSource`) has no `type: 'file'` member at all, confirmed by inspecting the installed `@anthropic-ai/sdk` package's own `.d.ts`. `RealAnthropicClient` now routes to `client.beta.messages.create({ ...params, betas })` only when `betas` is non-empty (verified: `POST /v1/messages?beta=true`, same path as the stable endpoint, not a different one), casting the beta response back to the stable `AnthropicMessage`/`AnthropicStreamEvent` aliases at that one boundary — every existing caller that never passes `betas` gets the exact prior behavior, zero blast radius. `citations` on a document block is already stable/GA (no beta needed) — only a Files-API `file_id` reference needs this. This feature is the first real (non-upload) consumer of Files API content blocks that `content-block-builder.md` documented but never actually exercised end-to-end; document that module's `files-api` mode now depends on passing `betas: ['files-api-2025-04-14']` into `createMessage`/`streamMessage` alongside the built block, or the request 400s.
- **[permanent-doc: `test-doubles.md`]** `FakeAnthropicClient`'s unqueued-call fallback gained two additions, both purely additive (only change behavior for a request shape no existing lab sends): (1) it now also recognizes a schema-less tool (`'type' in tool` starting with `text_editor_`, no `input_schema`) — not just a custom JSON-schema tool — and synthesizes a `create` call on a fixed `/notes.md` path, so a live fake-mode click exercises this lab's tool loop instead of always short-circuiting to plain text (this lab's text-editor tool has no `input_schema`, so the existing `isCustomTool()`/`pickFallbackTool()` heuristic never matched it). (2) it now fabricates one `page_location` citation on its plain-text fallback whenever the request's `messages` contain a `document` block with `citations: { enabled: true }`, so a citations-enabled lab has something real to render without a real Claude API call. Both only apply to the **non-streaming** fallback path's *final* text answer; the streaming fallback (`fallbackStreamEvents`) gained the same tool-call synthesis (shared via `fallbackToolCall()`) but not citation synthesis — reconstructing a `citations_delta` streamed event and threading it through would have meant extending this lab's own `accumulateStreamedContent()` too, judged out of proportion for what the streaming E2E path actually needs to prove (the tool loop + notes update, not citations specifically). The E2E spec accordingly only asserts a citation marker on the non-streaming turn.
- **[technical: `architecture.md`'s "Server-owned session state"]** Confirmed the exact design for how `deliveryMode`/caching interact across a session's multiple `ask` calls, since the plan's own wording ("on the session's first ask... reused on every later ask that requests files-api mode again") was ambiguous between "the document is attached once, ever" and "the document is resolved fresh every call." Resolved as: the document content block for `messages[0]` is *rebuilt every ask* from that ask's requested `deliveryMode` (reusing the cached `file_id` when switching back to `files-api`, never re-uploading), and `messages[0]` is fully replaced each time — not appended to. `markBreakpoints` is likewise reapplied on every call, not just the first, since a cache *read* requires `cache_control` to be resent on the identical prefix every time, not merely set once. The payoff: keeping the same delivery mode across consecutive asks yields a cache **read** (byte-identical `messages[0]`); toggling delivery mode mid-session deliberately produces a cache **write** instead on the very next call, since `messages[0]`'s bytes changed — a live, in-app demonstration of "changing an earlier region invalidates every region after it," not just an assertion in a doc. Worth folding into `architecture.md`'s own session-state section as the concrete example of a session field that gets *rebuilt* each call (the document block) versus one that's *appended* each call (the growing question/answer history).
- **[technical]** `backend/package.json` gained `fast-xml-parser` (a small, dependency-free XML parser) to parse the arXiv Atom API's response in `RealArxivClient`. No XML parser existed anywhere in this repo before; regex-parsing Atom XML by hand was judged too fragile for a real external API response.
- **[technical: `architecture.md`'s "Server-owned session state"]** A citation-bearing response block is never safely resendable as-is in a later request's history — confirmed against the real API with two distinct 400s from two successive (insufficient) fixes: first `citations.0.page_location.file_id: Extra inputs are not permitted` (the response-only `file_id` field isn't part of the citation *param* shape), then, after stripping only that field, `Invalid citation indices: Document not found for placeholder citation` (the remaining `document_index` placeholder still doesn't reliably re-resolve against a later request's document list — reproduced with delivery mode held constant throughout, so a changing document identity is not the trigger; the precise server-side rule isn't documented and wasn't independently confirmed beyond these two observed failures). Fixed by dropping citation metadata entirely (`toHistoryContent()`) when a turn's response is folded into session history for a later call — the cited *text* is kept (so Claude still remembers what it said), only the citation annotations are stripped. Worth folding into `architecture.md`'s session-state guidance as a general rule: strip citations from any response block before storing it as resendable history, full stop — not a partial-field fix, and not conditional on anything else about the request changing between calls.
- **[flag for follow-up — not fixed here, discuss and scope separately]** This feature's `accumulateStreamedContent()` (its own lab-local streamed-response reconstruction, per `envelope-builder.md`'s "stays lab-local" note) originally didn't accumulate `thinking_delta`/`signature_delta` events, only `text_delta`/`input_json_delta` — a streamed turn where Claude emitted a `thinking` block got reconstructed with an empty `thinking` field. Harmless until that (now-corrupted) block was resent as history on a later call, at which point the real API rejected it: `messages.N.content.0.thinking: each thinking block must contain thinking`. Fixed here (this file's own copy). **`backend/src/messages-console/messages-console.service.ts` and `backend/src/live-tool-use-console/live-tool-use-console.service.ts` both have byte-for-byte the same hand-rolled reconstruction pattern and the identical gap, confirmed by inspection — neither has been touched.** Both are exposed in principle: Messages Console has the frontend resend full turn history on every call, and Live Tool-Use Console resends its own turn's history mid-loop across tool round trips; either could hit the same 400 if Claude emits a thinking block during a streamed turn. Left alone since fixing another already-graduated work item's code is outside this feature's scope — needs its own follow-up task (fix in both places, or promote the reconstruction helper to a shared one so the fix isn't triplicated a third time).
- **[flag for follow-up — not fixed here, discuss and scope separately]** Hit live via a user bug report, not by design: `accumulateStreamedContent()` also didn't handle `citations_delta` — only `text_delta`/`thinking_delta`/`signature_delta`/`input_json_delta` had a case — so a streamed ask's text block never accumulated its citations, and `flattenCitations()` always returned `[]` for a streamed turn regardless of mode. Separately, `FakeAnthropicClient`'s `fallbackStreamEvents()` never fabricated a citation to begin with, unlike `fallbackMessage()`'s non-streaming fallback (see the test-doubles note above — its stated reason for skipping citation synthesis on the streaming fallback is superseded by this fix, not still accurate). Both fixed here: added the `citations_delta` case to this file's `accumulateStreamedContent()`, and extracted a shared `fabricatedCitation()` helper in `fake-anthropic-client.ts` so `fallbackStreamEvents()` fabricates the same citation `fallbackMessage()` does; added a `streamAsk` unit test asserting a `citations_delta` event flattens the same as a non-streamed response. **The part not fixed, worth scoping properly at graduation or later**: this is the second delta kind (after `thinking`/`signature`, per the note above) found missing from this hand-rolled reconstruction only by hitting it live — every SDK delta type needs its own manually-added `if` branch, with nothing forcing every kind to be handled, and the same duplicated reconstruction pattern exists byte-for-byte in `messages-console.service.ts` and `live-tool-use-console.service.ts` (per the note above, still untouched there). A shared reassembly helper — e.g. a switch/reducer keyed exhaustively off the delta's `type`, so an unhandled union member is a compile error rather than a silent drop — would remove this class of bug entirely instead of relying on someone noticing empty output after the fact. Scope this alongside promoting the reconstruction helper already flagged above, rather than patching a fourth delta kind into three separate copies again next time.
- **[technical]** `buildInitialParams()` sends a `system` prompt (a static constant, `SYSTEM_PROMPT`) — missing entirely from the original implementation. The text-editor tool (`ToolTextEditor20250728`) has no `description` field at all (unlike a custom tool, which explains itself via its own schema's `description`), so with no system prompt, Claude had no way to know `/notes.md` exists, what it's for, or that it's expected to use the tool when asked to note something — reported live: asking Claude to note something never triggered the tool, silently, with no error. Diagnosed from the code (confirmed no `system` field was ever sent) rather than reproduced directly; a one-line system prompt naming the file and when to use it addresses the identified cause, pending the user's own retest. General takeaway worth remembering for any future lab offering a schema-less/server-tool-shaped tool (text editor, bash, computer use, etc.): unlike a custom tool, these carry no room to explain themselves in the request — a system prompt is the only place to do it, not optional the way it might be for a self-describing custom tool.
