...
- **`GET /api/mode` extension:** `task-fake-mode.md`'s existing route gains `keyStatus: 'valid' | 'invalid'`, present in the response only when `fakeMode` is `false` (mirroring how `repoUrl` is present only when relevant to fake mode). Final shape: `{ fakeMode: boolean, repoUrl?: string, keyStatus?: 'valid' | 'invalid' }`.
...
- [ ] Extend `GET /api/mode` (from `task-fake-mode.md`) to include `keyStatus: 'valid' | 'invalid'`, present only when `fakeMode` is `false`.
