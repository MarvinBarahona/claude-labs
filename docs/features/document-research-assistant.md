...
`frontend/src/app/document-research-assistant/` (`DocumentResearchAssistant`). Stacks `<app-docs-panel [slug]="'document-research-assistant'" />` → the demo → `<app-inspector-panel [call]="inspectorCall()" />`, per the app-shell composition convention. The demo: an arXiv-ID entry form that starts a session and shows the paper's title/authors/summary once fetched; the shared `<app-chat-transcript>` (see `chat-transcript.md`) for the question/answer transcript (streaming toggle, delivery-mode toggle above it), supplying a custom `answerBodyTemplate` that renders each paragraph markdown-rendered plus a citation marker per citation — a small inline SVG "info" icon, not a numbered `[1]`/`[2]` (citations are almost always exactly one per block, so numbering added noise without conveying anything) — that discloses the cited text/page range on click; a running-notes side panel rendering the latest `notes` value (markdown-rendered via the shared `renderMarkdown()`), updating after each ask, capped at `max-h-[50vh]` with its own scrollbar. The transcript and notes panel stay mounted with skeleton placeholders while an ask is in flight rather than blanking.
...
- `document-research-assistant.spec.ts` (frontend unit) — the arXiv-ID form starting a session and rendering the fetched paper; the transcript rendering question/answer turns, each paragraph markdown-rendered, with citation markers from a mocked response; toggling delivery mode re-issuing the current question with both content-block shapes shown in the inspector; the notes panel rendering after each ask, confirmed markdown-rendered via the shared `renderMarkdown()`; the transcript/notes-panel skeletons holding for the minimum duration and never blanking on a second-onward ask.
