...
`frontend/src/app/core/lab-catalog.ts` exports `LabCatalogEntry` (`{ goal, concepts }`) and `LAB_CATALOG`, a `Record<string, LabCatalogEntry>` keyed by slug — one entry per graduated lab. `Home` spreads `LAB_CATALOG[feature.slug]` unguarded for every non-`home` `FEATURE_ROUTES` entry, so an entry must exist the moment a feature is registered in `FEATURE_ROUTES` (see `app-shell.md`'s "Using it") or `Home` throws rather than rendering blank — only its wording is refined later, by the `write-lab-doc` skill run against the lab's finished code. Unlike `FEATURE_ROUTES`, this registry has no ordering concern of its own; `Home` derives display order from `FEATURE_ROUTES` itself.
