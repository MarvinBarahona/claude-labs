...
Both tools are offered on every call (both custom, backend-executed). A `SYSTEM_PROMPT` restricts Claude to only the weather and configured-repo topics, declining anything else — without it, nothing stops Claude from answering unrelated questions without touching either tool, which would defeat the point of a tool-use console:
