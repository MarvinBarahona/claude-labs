...
- `data-code-sandbox.spec.ts` (frontend) — unit tests with `HttpTestingController`: the prompt form and skill checkbox, Run disabled on an empty prompt, Claude's final text reply rendering as markdown (and being omitted when the response has no text block), executed-code/stdout/stderr rendering, an image output file rendering inline vs. a non-image one rendering a download link, the `skillUsed` badge reflecting the mocked response, and the results-view skeleton holding for the minimum duration.
