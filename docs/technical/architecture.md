...
- **A server-executed tool's own response content blocks aren't a reliable count of how many times it ran.** Nesting under another server tool (e.g. a search tool whose `allowed_callers` routes it through a code-execution call, so its blocks carry a `caller` field and sit nested inside that call's own result rather than at the top level of `response.content`), or an opt-in response-inclusion exclusion, can each drop blocks from the top level or the response entirely while the call is still billed. `usage.server_tool_use` carries the authoritative per-tool-type billed count — count from there, not from `response.content`, whenever the actual number of calls matters.
