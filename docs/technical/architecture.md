...
- **A schema-less custom tool carries no room to explain itself.** A server-tool-shaped custom tool (the text-editor, bash, or computer-use tool families) has no `description` field the way a self-describing custom tool's own JSON-schema definition does, so Claude has no way to learn what it's for or when to use it from the tool definition alone. Any feature offering one of these must explain it via the request's `system` prompt instead — omitting one leaves Claude unaware the tool applies, silently, with no error signal.

## Unbounded external data sources

An external source with no inherent bound on response size (a full recursive file tree, a whole file's contents, an unpaginated list, a large PDF or image) can hand back an amount of data whose token cost — and, once it's sitting in conversation history, whose *repeated* cost on every later call — dwarfs everything else in a turn. Which mitigation applies depends on whether the data can be cut down without breaking what it's for:

- **A custom-tool result truncates safely** — cap entries/characters returned and signal when it truncated (e.g. a `truncated: boolean` field), rather than either propagating the source's raw size unbounded or truncating silently. A signaled truncation lets the model act on a partial result instead of mistaking it for a complete one.
- **A document/image content block doesn't truncate safely** — cutting it down can invalidate structure Claude derives from the whole content (a PDF's page-based citations, for instance, no longer line up against a cut-down file). Here the mitigation is a warning surfaced to the user once the source's size is known, not a silent cap and not blocking the request outright — the user decides whether to proceed, informed that this particular input costs and runs slower than usual.
...
- **A shared client's error type carrying no HTTP status code (e.g. `ExternalApiError`) can't be used to distinguish "not found" from a genuine transport failure (rate limit, auth, 5xx) after the fact.** A custom tool backed by a per-argument lookup (fetching one specific item by an ID or path Claude itself supplied, as opposed to fetching a whole list) should treat *any* failure from that lookup as the tool's own `is_error: true` result rather than letting it propagate — the failure is overwhelmingly likely to be "that argument doesn't resolve to anything," and forcing a hard 502/mid-stream error over a single bad guess kills the whole turn for what's usually a recoverable miss. A tool backed by a whole-collection fetch (no per-call argument narrowing which item comes back) has no such ambiguity and should keep letting a shared client's failure propagate as a genuine transport error, per the bullets above.
