...
- **A custom tool wrapping a size-unbounded external data source must cap what it returns.** An external API with no inherent bound on response size (a full recursive file tree, a whole file's contents, an unpaginated list) can hand back an amount of data whose token cost dwarfs everything else in a turn — worse, that cost repeats every later call in a loop once the result is sitting in conversation history. A custom tool built on a source like this caps entries/characters returned and signals when it truncated, rather than either propagating the source's raw size unbounded or truncating silently — a signaled truncation lets the model act on a partial result instead of mistaking it for a complete one.
