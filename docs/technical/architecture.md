...
Where any of these apply, the standing shape is a service-level in-memory `Map<sessionId, ...>` holding whatever that feature's own session needs. No database, no persistence across a process restart — consistent with every other piece of app state (e.g. `FakeGithubClient`'s canned data) being in-memory only; this app has no persistence layer anywhere. A page refresh starts a new session rather than resuming a stale one — deliberate, not an oversight, since there's no resume affordance anywhere else in the app either. A feature that doesn't need any of the three reasons above stays stateless, same as most of the app.

A session field falls into one of two update patterns per call, and picking the wrong one silently breaks caching: a field that resolves an attachment or resource from the current call's own request options (e.g. which delivery mode to attach a document with) is rebuilt fresh on every call — reusing a cached reference (e.g. an already-uploaded file id) when the same option is requested again, but never re-uploading or re-resolving needlessly — while a field representing accumulated conversation history is appended to, never rebuilt. Any cache-control breakpoint placed on a rebuilt region must be reapplied on every call too, not just the first, since a cache *read* requires resending `cache_control` on an identical prefix every time, not merely setting it once; switching what an earlier region resolves to (e.g. changing delivery mode) deliberately produces a cache *write* on the very next call instead of a read, since that region's bytes changed.

A response content block carrying citations is never safely resendable as-is once folded into stored session history for a later call — the API rejects citation metadata tied to a document identity that no longer reliably resolves against a later request's document list (observed as two distinct 400s from two successive, insufficient fixes: a response-only field rejected as an unpermitted param, then an unresolvable placeholder document index once that field alone was stripped). Strip citation metadata from any assistant content block before storing it as resendable history — keep the cited *text* itself, so Claude still recalls what it said, but drop the citation annotations unconditionally, not only when something else about the request changed between calls.
