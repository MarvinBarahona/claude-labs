...
| Unbounded external data sources | Read when a lab's tool result, content block, or server-executed/MCP tool call might return data with no inherent size bound — which mitigation shape applies depends on which of those three carries it | [`unbounded-data-sources.md`](unbounded-data-sources.md) |
